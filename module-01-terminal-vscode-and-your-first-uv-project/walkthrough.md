...
Windows students work there for the whole course, and it's worth being concrete about why rather than asking you to take it on faith. The tooling this course is built on — `git`, `uv`, `ssh`, `rsync` — grew up on Unix. PowerShell has equivalents for some of it and not for the rest: connecting a coding agent to a remote host over SSH can need **connection multiplexing**, which Microsoft's OpenSSH port has never implemented (it fails with `getsockname failed: Not a socket`), and syncing files at scale wants **`rsync`**, which Windows does not ship at all. Neither has a native workaround. So rather than carry a second, more fragile set of instructions, we put Windows students on Ubuntu from day one — with the bonus that from here on, every command in every walkthrough means the same thing on your machine as on your classmate's Mac.
...
When you eventually run a long job on a remote machine, you'll want it to survive your connection dropping — either a job scheduler on the host, or **`tmux`**, a "terminal multiplexer" that keeps a shell session alive when the connection goes. Nothing to install now.
